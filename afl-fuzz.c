...
static void chooseNext_queue_cur(){
  ...
  struct queue_entry *prev_queue_cur = queue_cur;//先存一下当前的entry
  ...
  u32 bestEntry_paths                = getPaths(queue_cur->exec_cksum); //获得当前种子的路径频率

  u32 queued_paths_now = 0;
  queue_it = queue; // 从队列的头部开始遍历
  ...
  if (queued_paths_now != queued_paths) { // 后者是全局的数据，前者是当前最新数据
    queued_paths = queued_paths_now;
  }

  queue_it=queue_cur;

  while (queue_it->next) { //我在这里只看到了两个因素，因为执行时间、种子size 可能在 Top_rated 那里体现
    if (queue_it->next->fuzz_level < bestEntry_level //s(i)
        || (queue_it->next->fuzz_level == bestEntry_level
            && getPaths(queue_it->next->exec_cksum) < bestEntry_paths)) { //f(i)
  ...
  if (bestEntry->exec_cksum != queue_cur->exec_cksum){
    ...
    if (bestEntry->next)
      bestEntry_prev->next = bestEntry->next; //删除
    else if (bestEntry != queue_top) // queue_top 指的是链表的尾部
    ...
    //把bestEntry插入到queue_cur之前，然后让queue_cur指向bestEntry，表示当前要fuzz的是bestEntry
